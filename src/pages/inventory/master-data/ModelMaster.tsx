...
const ModelMaster: React.FC = () => {
  ...
  const fetchModels = async () => {
    try {
      ...
      const formattedData: FormattedModel[] = data.map((item: ModelItem) => ({
        id: item.id,
        name: item.name,
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const successMsg = modalMode === 'create' ? 'Model created successfully!' : 'Model updated successfully!';
  ...
  const rowActions: RowAction[] = [
    {
      ...
      onClick: (row) => handleViewModel(row)
    },
    {
      label: 'Edit',
      icon: 'edit',
      color: 'text-green-600 hover:text-green-800',
      onClick: (row) => handleEditModel(row)
    },
    {
      label: 'Delete',
      icon: 'delete',
      color: 'text-red-600 hover:text-red-800',
      onClick: (row) => handleDeleteModel(row)
    }
  ];

  const handleViewModel = (model: FormattedModel) => {
    navigate(`${location.pathname}?id=${model.id}&mode=view`);
    setFormData({ id: model.id, name: model.name, isActive: model.isActive === 'Active' });
    setModalMode('view');
    setShowModal(true);
  };

  const handleEditModel = (model: FormattedModel) => {
    navigate(`${location.pathname}?id=${model.id}&mode=edit`);
    setFormData({ id: model.id, name: model.name, isActive: model.isActive === 'Active' });
    setFormError(null);
    setModalMode('edit');
    setShowModal(true);
  ...
  const handleDeleteModel = async (model: FormattedModel) => {
    const result = await Swal.fire({
      title: 'Delete Model',
      html: `Are you sure you want to delete <strong>"${model.name}"</strong>? This action cannot be undone.`,
      icon: 'warning',
    ...
    try {
      Swal.fire({
        title: 'Deleting...',
        html: 'Please wait while we delete the model.',
        icon: 'info',
        ...
        didOpen: async () => {
          ...
          try {
            ...
            Swal.fire({
              title: 'Deleted!',
              text: `"${model.name}" has been deleted successfully.`,
              icon: 'success',
          ...
          <div className="bg-white rounded-lg shadow-lg max-w-md w-full mx-4">
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200">
              <h2 className="text-xl font-semibold text-gray-900">
                {modalMode === 'create' && 'Add New Model'}
                {modalMode === 'edit' && 'Edit Model'}
                {modalMode === 'view' && 'View Model'}
              </h2>
              <button
                onClick={closeModal}
            ...
            <form onSubmit={handleFormSubmit} className="p-6">
              ...
              <div className="mb-5">
                <label htmlFor="name" className="block text-sm font-medium text-gray-700 mb-2">
                  Model Name {modalMode !== 'view' && <span className="text-red-500">*</span>}
                </label>
                <input
                  type="text"
                  id="name"
