...
const TermsConditions = () => {
  ...
  const fetchTermsConditions = async () => {
    try {
      setLoading(true)
      const response = await fetch('http://localhost:5104/api/TermsConditions')
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (modalMode === 'create') {
        ...
        const response = await fetch(`http://localhost:5104/api/TermsConditions/${formData.id}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            id: formData.id,
  ...
  const handleViewTermsConditions = async (item: FormattedTermsConditions) => {
    try {
      const response = await fetch(`http://localhost:5104/api/TermsConditions/${item.id}`)
      if (response.ok) {
        const data: TermsConditions = await response.json()
        setFormData({
          id: data.id,
