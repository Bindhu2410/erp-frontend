...
const QuotationTitle = () => {
  ...
  const fetchQuotationTitles = async () => {
    try {
      setLoading(true)
      const response = await fetch('http://localhost:5104/api/QuotationTitle')
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (modalMode === 'create') {
        ...
        const response = await fetch(`http://localhost:5104/api/QuotationTitle/${formData.id}`, {
          method: 'PUT',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
            Id: formData.id,
    ...
    },
    {
      ...
      onClick: async (row) => {
        ...
        if (result.isConfirmed) {
          try {
            const response = await fetch(`http://localhost:5104/api/QuotationTitle/${row.id}`, {
              method: 'DELETE',
              headers: { 'Content-Type': 'application/json' }
            })
