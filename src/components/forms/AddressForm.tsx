    ...
    const fetchInitialData = async () => {
      const [states, territories, districts, cities, areas, pincodes] =
        await Promise.all([
          ...
          axios.get("http://localhost:5104/api/SalesCity"),
          axios.get("http://localhost:5104/api/SalesArea"),
          axios.get("http://localhost:5104/api/SalesPincode"),
        ]);
      setOptions((prev) => ({
        ...prev,
        states: states.data,
      ...
      switch (field) {
        case "state":
          const territories = await axios.get(
            `http://localhost:5104/api/SalesTerritory/state/${value}`
          );
          setOptions((prev) => ({ ...prev, territories: territories.data }));
          break;
        case "district":
          const cities = await axios.get(
            `http://localhost:5104/api/SalesCity/district/${value}`
          );
          setOptions((prev) => ({ ...prev, cities: cities.data }));
          break;
        case "city":
          const areas = await axios.get(
            `http://localhost:5104/api/SalesArea/city/${value}`
          );
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await axios({
        ...
        url: `http://localhost:5104/api/SalesAddress${
          editId ? `/${editId}` : ""
        }`,
        data: payload,
        headers: {
  ...
  const fetchaddressById = async (id: string) => {
    try {
      const response = await axios.get(
        `http://localhost:5104/api/SalesAddress/${editId}`
      );
